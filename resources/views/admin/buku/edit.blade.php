...
@section('title', 'Edit Buku - Sikutu')
...
<div class="card max-w-4xl animate-[fadeIn_0.3s_ease-in-out]">
    <form action="{{ route('admin.buku.update', $buku->uuid) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" id="kode_buku" name="kode_buku" value="{{ old('kode_buku', $buku->kode_buku) }}" required class="input-field" placeholder="Contoh: BK-001">
                ...
                <div>
                    ...
                    <input type="text" id="judul_buku" name="judul_buku" value="{{ old('judul_buku', $buku->judul_buku) }}" required class="input-field" placeholder="Masukkan judul buku">
                ...
                <div>
                    ...
                    <input type="text" id="pengarang" name="pengarang" value="{{ old('pengarang', $buku->pengarang) }}" required class="input-field" placeholder="Nama pengarang">
                ...
                <div>
                    ...
                    <input type="text" id="penerbit" name="penerbit" value="{{ old('penerbit', $buku->penerbit) }}" class="input-field" placeholder="Nama penerbit">
                ...
                <div>
                    ...
                    <input type="number" id="tahun_terbit" name="tahun_terbit" value="{{ old('tahun_terbit', $buku->tahun_terbit) }}" class="input-field" placeholder="Contoh: 2023" min="1900" max="{{ date('Y') }}">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="number" id="stok" name="stok" value="{{ old('stok', $buku->stok) }}" required class="input-field" min="0">
                ...
                <div>
                    ...
                    <x-custom-select name="kondisi" :options="['BAIK' => 'Baik', 'RUSAK' => 'Rusak', 'HILANG' => 'Hilang']" selected="{{ old('kondisi', $buku->kondisi) }}" placeholder="Pilih Kondisi" />
                ...
                <div>
                    ...
                    <div class="bg-gray-50 p-3 rounded border border-gray-200 max-h-40 overflow-y-auto">
                        @php
                            $selectedGenres = old('genres', $buku->genres->pluck('id_genre')->toArray());
                        @endphp
                        @foreach($genres as $genre)
                            <div class="flex items-center mb-2">
                                <input id="genre_{{ $genre->id_genre }}" name="genres[]" type="checkbox" value="{{ $genre->id_genre }}"
                                    class="w-4 h-4 text-blue-600 bg-white border-gray-300 rounded focus:ring-blue-500"
                                    {{ in_array($genre->id_genre, $selectedGenres) ? 'checked' : '' }}>
                ...
                <div x-data="imageUploader('{{ $buku->gambar_cover ? Storage::url($buku->gambar_cover) : '' }}')">
                    <label class="block text-sm font-medium text-gray-700 mb-1 font-oswald">Cover Buku Baru (Opsional)</label>
                    ...
                    <p class="text-xs text-gray-500 mt-2">Biarkan kosong jika tidak ingin mengubah cover.</p>
...
<script>
function imageUploader(initialImageUrl = '') {
    return {
        dragover: false,
